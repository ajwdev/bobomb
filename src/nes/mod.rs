...
impl Nes {
    pub fn new(rom_buffer: &[u8], program_counter: Option<u16>) -> Nes {
        ...
        let chr_data = if chr_bank_count > 0 && chr_end <= rom_buffer.len() {
            Some(rom_buffer[chr_start..chr_end].to_vec())
        } else {
            None
        };

        let mut ppu = ppu::Ppu::new(chr_data);
    ...
    fn rom_is_double_banked(rom: &[u8]) -> bool {
        ...
        match rom[4] {
            // TODO Is this panic worthy or should we assume 1? After all, you can't
